...
const exampleReviews = [
  {
    ...
    imageUrl: 'https://thai.monkeytravel.com/attach/product/MONKEY/202306/o_1h27nbv6i1kv716qi1nsjuv6tvjq.jpg?w=500&h=0'
  },
  {
    id: 2,
    ...
    imageUrl: 'https://www.agoda.com/wp-content/uploads/2020/01/Things-to-do-in-Dubrovnik-Picturesque-view-of-Dubrovnik.jpg' 
  },
  {
    id: 3,
    ...
    package: "EPP3737",
    itinerary: "유럽",
    content: `작년에 산티아고 패키지 여행을 통해 참좋은여행사에 대해 처음 알았고, 경험을 하였습니다.
그때 경험이 괜찮다고 느껴, 올해 여행도 참좋은 여행사로 진행하려 했었고,
결정적으로는 이탈리아 여행을 찾고 있었는데 돌로미티가 포함된 상품이 있어서 였습니다.
베니스 In Out 아시아나 비행기를 이용한 비행편은 아주 좋았어요.
첫번째 여행지 베르나 부터 마지막 베니스까지 알찬 여행이었다고 생각됩니다.
이번 여행은 바다, 산, 예전 고대의 건물들 모두를 볼 수 있었는데,
바다는 소렌토의 절벽이 있는 바다, 카프리 섬의 청록색 바다가 인상깊었습니다.
산으로는 돌로미티였는데, 쉽게 볼 수 없는 광경이었습니다.
위를 보면 바위산이 보이고, 발아래는 넓은 풀 밭이 펼쳐져있습니다.
로마의 바티칸시국과 콜로세움, 발도르차의 사이플러스 나무가 생각나는데,
여행 복귀후에 글래디에이터를 다시 봤는데,
그 현장에 내가 있는 것 같은 생각이 들었고, 과거로 타임머신을 타고 막 다녀왔다는
느낌이 들었습니다.
이번 여행에 수고해 주신 가이드님 및 여행팀분들, 감사합니다^^`,
...
설렘과 기대감으로 시작한 여행은 아름다움과 경이로움의 연속이었습니다. 아직도 스위스의 정경이 눈에 아른거립니다.

아주 오랜만의 패키지 여행이라 걱정도 있었지만, 따뜻한 리더십을 가진 '주덕근' 가이드님의 배려 속에서 좋은 분들과 함께 추억을 쌓을 수 있어 자유여행과 다른 특별한 경험을 할 수 있었습니다(함께 하신 분들 벌써 보고 싶네요).
...
스위스는 차량으로 이동할 때 창밖의 풍경마저도 너무 아름다워 잠깐이라도 눈을 붙일 수가 없었습니다!

더불어 이 여행을 더욱 빛내 주신 주덕근 가이드님께 다시 한 번 감사의 말씀을 드리고 싶습니다. 
...
const ReviewList = () => {
  ...
  const toggleExpand = (id) => {
    setExpandedReviewIds((prevExpandedReviewIds) => {
      const newExpandedReviewIds = new Set(prevExpandedReviewIds);
      if (newExpandedReviewIds.has(id)) {
        newExpandedReviewIds.delete(id);
      } else {
        newExpandedReviewIds.add(id);
      }
      return newExpandedReviewIds;
    });
  };
  ...
  return (
    <>
                ...
                <div className={`review-content ${expandedReviewIds.has(review.id) ? 'expanded' : ''}`}>
                  ...
                  <div className="review-body">
                    {expandedReviewIds.has(review.id) ? (
                      <>
                        {review.content}
                        <Button size="small" onClick={() => toggleExpand(review.id)}>닫기</Button>
                      </>
                    ) : (
                      <>
                        {review.content.substring(0, MAX_LENGTH)}
                        {review.content.length > MAX_LENGTH && (
                          <Button size="small" onClick={() => toggleExpand(review.id)}>더보기</Button>
                        )}
                      </>
                    )}
                  </div>
      ...
      <div className="fixed-button">
        <Link to="/CreateTripReview">후기 작성하기</Link>
      </div>
