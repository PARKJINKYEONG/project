...
const ReviewList = () => {
  ...
  useEffect(() => {
    // Initialize image indexes on mount
                    ...
                    {images.length > 0 ? (
                      <>
                        ...
                        <div className="thumbnail-navigation">
                          <button onClick={() => handlePrevThumbnails(review.id)} disabled={(currentThumbnailPageIndexes[review.id] ?? 0) <= 0}>◀</button>
                          <button onClick={() => handleNextThumbnails(review.id)} disabled={((currentThumbnailPageIndexes[review.id] ?? 0) + 1) * THUMBNAILS_PER_PAGE >= images.length}>▶</button>
                        </div>
                      </>
                    ) : (
