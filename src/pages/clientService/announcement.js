...
function Announcement() {
  ...
  useEffect(() => {
    const fetchNotices = async () => {
      try {
        const response = await axios.get('http://localhost:8080/api/notice/all/user');
        console.log(response.data);  // 데이터 확인
        setNotices(response.data);
      } catch (error) {
        console.error('공지 목록을 불러오는 중 오류가 발생했습니다.', error);
      }
    };
    fetchNotices();
  }, []);
              ...
              notices.map((notice, index) => (
                <tr key={notice.id}>
                  ...
                  <td className="col-4 text-center">
                    <Link to={`/announcement/${notice.id}`} className={styles.noticeLink}>
                      {notice.title}
                    </Link>
                  </td>
                  <td className="col-2 text-center">{notice.noticeDate.slice(0, 10)}</td>
                  <td className="col-2 text-center">{notice.writer}</td>
                  <td className="col-2 text-center">{notice.viewCount}</td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="5" className={`text-center ${styles.noData}`}>
                  등록된 공지 사항이 없습니다.
                </td>
              </tr>
            )}
